#!/usr/bin/env python3
"""
MLB Predictor v3.1 - ML INTEGRATED
===================================
Real ML predictions using trained XGBoost models.
Features:
- Uses trained hit_model_xgb and hr_model_xgb
- Full Statcast feature engineering with REAL stats (v3.1 fix)
- Live game data integration
- Enhanced probability calculations

Author: Mike Ross
Date: 2026-02-21
Version: 3.1 - Fixed stats calculation to use real Statcast data
"""
...
class MLBPredictor:
    ...
    def get_features(self):
        """Return feature list for prediction"""
        return [
            'release_speed', 'release_spin_rate', 'release_extension',
            'pitch_type_code', 'zone', 'balls', 'strikes', 'outs_when_up', 'inning',
            'on_1b', 'on_2b', 'on_3b',
            'launch_speed', 'launch_angle', 'hc_x', 'hc_y',
            'stand_r', 'p_throws_r', 'inning_top',
            'delta_run_exp', 'delta_home_win_exp',
            'n_thruorder_pitcher', 'pitcher_days_rest',
            'bat_speed', 'swing_length', 'attack_angle'
        ]
        
    def prepare_features(self, row):
        """Convert a row to feature vector for ML model"""
        features = {}
        ...
        features['pitch_type_code'] = row.get('pitch_type_code', 2)
        ...
        features['inning_top'] = int(row.get('inning_topbot', 'Top') == 'Top')
        ...
        # Matchup
        features['stand_r'] = int(row.get('stand', 'R') == 'R')
        features['p_throws_r'] = int(row.get('p_throws', 'R') == 'R')
        ...
        features['pitcher_days_rest'] = row.get('days_rest', 4)
    ...
    def predict(self, features_df):
        ...
        X = features_df[self.get_features()].fillna(0)
...
def calculate_pitcher_stats(statcast_df, pitcher_name, team):
    ...
    # Try to find pitcher by name or team
    if pitcher_name and pitcher_name != "TBD":
        pitcher_data = statcast_df[
            (statcast_df['player_name'].str.contains(pitcher_name.split()[-1], case=False, na=False)) &
            (statcast_df['home_team'].isin([team, 'NYY', 'BOS', 'LAD', 'SF', 'CHC', 'STL', 'HOU', 'TEX', 'NYM', 'PHI', 'ATL', 'MIA', 'SD', 'ARI', 'COL', 'MIL', 'CIN', 'CLE', 'MIN', 'DET', 'KC', 'CWS', 'TB', 'BAL', 'TOR', 'OAK', 'SEA', 'LAA', 'PIT', 'WSH']) | 
             statcast_df['away_team'].isin([team, 'NYY', 'BOS', 'LAD', 'SF', 'CHC', 'STL', 'HOU', 'TEX', 'NYM', 'PHI', 'ATL', 'MIA', 'SD', 'ARI', 'COL', 'MIL', 'CIN', 'CLE', 'MIN', 'DET', 'KC', 'CWS', 'TB', 'BAL', 'TOR', 'OAK', 'SEA', 'LAA', 'PIT', 'WSH']))
        ]
    else:
        # Use team data
        pitcher_data = statcast_df[
            (statcast_df['home_team'] == team) | (statcast_df['away_team'] == team)
        ].sample(min(500, len(statcast_df)), random_state=42)
...
def calculate_batter_stats(statcast_df, batter_name, team):
    ...
    if batter_name and "Batter" not in batter_name:
...
def generate_predictions():
    ...
    print("\n" + "="*60)
    print("⚾ MLB PREDICTOR v3.1 - ML INTEGRATED (Real Stats Fixed)")
    print("="*60)
    ...
    for matchup in matchups:
        ...
        home_batter_stats = calculate_batter_stats(statcast_df, f"Batter {home}", home)
        ...
        pred = {
            ...
            'hit_probability': round(ml_hit_prob * 100, 1),
        ...
        away_batter_stats = calculate_batter_stats(statcast_df, f"Batter {away}", away)
        ...
        pred_away = {
            ...
            'hit_probability': round(ml_hit_prob_a * 100, 1),
